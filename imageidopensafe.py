...
def checksafe(overlay):
    wallet = apps_data[0][1]
    wallet = w3.toChecksumAddress(wallet)

    req = requests.get(f'https://safe-transaction-polygon.safe.global/api/v1/owners/{wallet}/safes/')
    t = json.loads(req.content)
    print(t)
    print(t['safes'])
    safe = t['safes'][0]
    ...
    req = requests.get(f'https://safe-transaction-polygon.safe.global/api/v1/safes/{safe}/multisig-transactions/')
    t = json.loads(req.content)
    print(t)
    print(t['results'])
    parameters = t['results']
    print(type(parameters))
    print(parameters[0]['isExecuted'])

#        im = Image.new("RGB", (240, 240), "red")
#        d = ImageDraw.Draw(im)
#        disp.image(im)

    if parameters[0]['isExecuted'] == False:
 
#        im = Image.new("RGBA", (240,240), color=(0,0,0,0))
#        d = ImageDraw.Draw(im)
        d = ImageDraw.Draw(overlay)                
        border_width = 5
        d.rectangle((0,0, 240, 240), outline='greenyellow', width=border_width)
#        d.rectangle((0,0, 240-border_width, 240-border_width), outline='greenyellow', width=border_width)
