...
def swap_tokens(disp, wallet_address, private_key, from_token, to_token, w3):
    ...
    try:
        ...
        if balance == 0:
            error_message = f"No {config.L2_token_name if not is_native_input else 'ETH/MATIC'} to swap"
            display_message(disp, error_message, is_error=False)
            print(error_message)  # Print to console for debugging
            return None
        
        # Calculate amount to swap (existing logic)
        if is_native_input:
...
def swap_l2_token_to_output(disp, get_address, get_private_key):
    config = load_config()
    return perform_swap(disp, get_address, get_private_key, config.L2_token_address, config.L2_weth_address)

def swap_output_to_l2_token(disp, get_address, get_private_key):
    config = load_config()
    return perform_swap(disp, get_address, get_private_key, config.L2_weth_address, config.L2_token_address)
