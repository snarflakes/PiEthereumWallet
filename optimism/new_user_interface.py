...
def initialize_config():
    """Initialize config from active chain on startup"""
    try:
        active_chain = get_active_chain()
        if active_chain != "unknown":
            chain_config = load_chain_config(active_chain)
            if chain_config:
...
def load_chain_config(chain_dir):
    """Load config.json from specified chain directory"""
    try:
        config_path = os.path.join(chain_dir, 'config.json')
        with open(config_path, 'r') as f:
            return json.load(f)
    except Exception as e:
        print(f"Error loading config from {chain_dir}: {e}")
        return None
...
#swap menu
SWAP_MENU = [f"{config.L2_token_name} to {config.L2_output_name}", 
    ...
    try:
        ...
        for config_file in config_files:
            ...
            try:
                with open(config_path, 'r') as f:
                    cfg = json.load(f)
                    display_name = f"{cfg['L2_token_name']} to {cfg['L2_output_name']}"
                    pairs_menu.append({
...
def display_menu():
    ...
    # Draw the title
    draw.text((WIDTH//2, 20), title, font=font, fill='black', anchor="mm")
    ...
    draw.text((10, HEIGHT - 32), "↑↓: Navigate", font=small_font, fill='black')
    draw.text((WIDTH//2, HEIGHT - 32), "→: Select", font=small_font, fill='black', anchor="mm")
...
def handle_button_select():
    ...
    if current_menu == MAIN_MENU:
        ...
        if selected_option == "Select Chain":
            ...
            if chain_menu:  # Only change menu if we got valid chain options
        ...
        if selected_pair['display'] == "Back":
            ...
            try:
                # Save the selected config for the chain
                save_active_config(selected_pair['chain_dir'], selected_pair['config_file'])
                # Reload the configuration
                config = initialize_config()
                if config:
                    # Show swap direction menu instead of executing swap immediately
                    current_menu = [
    ...
    elif current_menu == [f"{config.L2_token_name} to {config.L2_output_name}",
                         f"{config.L2_output_name} to {config.L2_token_name}",
                         "Back"]:  # Swap direction menu
        if selected_option == "Back":
            current_menu = get_token_pairs_menu()
            selected_index = 0
        else:
...
def execute_swap_option(option):
    ...
    if option == f"{config.L2_token_name} to {config.L2_output_name}":
        # Swap from L2_token to L2_output
        ...
        # Swap from L2_output to L2_token
