...
class HomeScreen:
    ...
    def render_home_screen_l2(self):
        ...
        self.add_wallet_text(draw, self.config.L2_name, wallet)
 ...
 # Truncate the wallet address
    ...
    def get_token_balances(self, w3, address):
        ...
        # For native tokens (ETH/MATIC/etc), use get_balance instead of contract call
        if self.config.L2_output_name.upper() in native_tokens:
            eth_balance = w3.eth.get_balance(address) / 10**18  # All native tokens use 18 decimals
        else:
            eth_contract = w3.eth.contract(address=self.config.L2_output_address, abi=self.config.L2_output_contractABI)
            eth_balance = eth_contract.functions.balanceOf(address).call() / self.config.L2_token_decimal_output

        # Get USDC or other input token balance
        token_contract = w3.eth.contract(address=self.config.L2_token_address, abi=self.config.L2_token_contractABI)
        token_balance = token_contract.functions.balanceOf(address).call() / self.config.L2_token_decimal
    ...
    def draw_balance_info(self, draw, native_balance, eth_balance, token_balance):
        font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 18)

        # First line: Display input token balance (USDC)
        draw.text((60, 205), f"{self.config.L2_token_name}: {round(token_balance, 4)}", fill="black", anchor="ms", font=font)

        # Second line: Display output token balance (wBTC in this case)
        if self.config.L2_output_name == "ETH" and self.config.L2_chainid != "137":
            draw.text((60, 225), f"{self.config.L2_output_name}: {round(Web3.fromWei(native_balance, 'ether'), 5)}", fill="black", anchor="ms", font=font)
        else:
            # For wBTC as output token, use scientific notation for small amounts
            if self.config.L2_output_name.upper() == "WBTC" and eth_balance < 0.00001:
                output_display = f"{eth_balance:.2e}"  # Scientific notation with 2 decimal places
            else:
                output_display = f"{round(eth_balance, 8)}"  # Show 8 decimal places for regular display
            
            draw.text((60, 225), f"{self.config.L2_output_name}: {output_display}", fill="black", anchor="ms", font=font)

        # Debug prints
        print(f"Input token name: {self.config.L2_token_name}")
        print(f"Input token balance: {round(token_balance, 4)}")
        print(f"Output token name: {self.config.L2_output_name}")
        print(f"Output token balance: {eth_balance:.2e}")
