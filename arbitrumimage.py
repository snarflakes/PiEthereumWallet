...
def refresh():
    if internet():
        ...
        d.text((55, 200), "Arbiscan", fill="black", anchor="ms", font=font)
...
# Display qrcode link to etherscan wallet info
        ...
        qr = qrcode.QRCode()
        qr.add_data(f"https://arbiscan.io/address/{(apps_data[0][1])}")
#    qr.add_data(wallet)
        qr.make()
...
#transparency for boxes(gas costs?)
    ...
    font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 18)
    img = Image.new("RGBA", (240, 240), "blue")
...
# print current ETH account value
    balance = web3.eth.getBalance(acct.address)
    print('Account Balance ETH: ', (web3.fromWei(balance, 'Ether')))
...
# add qrcode to right corner
    qr = qrcode.QRCode()
    qr.add_data(f"https://arbiscan.io/tx/{web3.toHex(tx_hash)}")
#    qr.add_data(wallet)
    qr.make()
    imgrender = qr.make_image(fill_color="black", back_color="#FAF9F6")
...
#add in a image layer ontop of white background, way of eliminated transparent layer too
    ...
    image = picture_1.resize((50, 50))
    img.paste(image, (50, 50))
    d.text((70, 114), "         Arbitrum Wallet ", fill="black", anchor="ms", font=font, stroke_width=1, stroke_fill='white')
...
#    draw.rectangle(((10, 200), (100, 240)), fill=TINT_COLOR+(OPACITY,))
    ...
    draw.ellipse((130, 30, 220, 100), fill = 'white', outline ='white')
...
#        im = Image.new("RGB", (150, 50))
    d = ImageDraw.Draw(overlay)                
    d.text((60, 210), "ETH:" + str(round(web3.fromWei(balance, 'Ether'), 4)), fill="black", anchor="ms", font=font)
    d.text((60, 230), str(token_declare.functions.symbol().call()) + ":" + str(round(token_balance/1000000, 2)), fill="black", anchor="ms", font=font)
    d.text((170, 70), "Gas " + str(t['result']['FastGasPrice']), fill="black", anchor="ms", font=font)
